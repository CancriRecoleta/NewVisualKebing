...
public class KeybindBindBoardScreen extends FixedScaleScreen {
    ...
    private void renderHeader(GuiGraphicsExtractor g) {
        ...
        g.text(font, title.getString(), 12, (HEADER_H - font.lineHeight) / 2, c.textPrimary(), true);
    }
    ...
    private void renderCollapsedRail(GuiGraphicsExtractor g, int x, int top, int h, int mouseX, int mouseY) {
        var c = UITheme.colors();
        ...
        String chev = "▸";
        g.text(font, chev, r[0] + (r[2] - font.width(chev)) / 2,
        ...
        for (int i = 0; i < hint.length() && gy + font.lineHeight < top + h - 4; i++) {
            ...
            g.text(font, ch, cx - font.width(ch) / 2, gy, UITheme.withAlpha(c.textMuted(), 0xC0), false);
            gy += font.lineHeight;
        }
    }

    /** Collapse/expand chevron drawn in the expanded panel's header corner. */
    private void renderPanelToggle(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        var c = UITheme.colors();
        int[] r = panelToggleRect();
    ...
    private void renderFilterBar(GuiGraphicsExtractor g, int x, int w, int mouseX, int mouseY) {
        var c = UITheme.colors();
        ...
        g.text(font, KeybindViewerScreen.fitToWidth(font, label, w - 16),
    ...
    private void renderModDropdown(GuiGraphicsExtractor g, int x, int w, int mouseX, int mouseY) {
        var c = UITheme.colors();
        ...
        for (int i = modDropdownScroll; i < ids.size() && i < modDropdownScroll + visible; i++) {
            ...
            g.text(font, KeybindViewerScreen.fitToWidth(font, name, w - 14),
    ...
    private void renderCategoryRow(GuiGraphicsExtractor g, CategoryEntry ce, int x, int y, int w, int mouseX, int mouseY) {
        var c = UITheme.colors();
        ...
        g.text(font, chev, x + 2, textY, hovered ? c.textPrimary() : c.textSecondary(), false);
        ...
        g.text(font, name, labelX, textY, c.accentLight(), false);
        g.text(font, count, x + w - countW - 2, textY, UITheme.withAlpha(c.textMuted(), 0xC8), false);
    }

    private void renderFuncRow(GuiGraphicsExtractor g, FuncEntry fe, int x, int y, int w, boolean hovered, boolean selected) {
        var c = UITheme.colors();
        int rowH = ROW_H - 2;
        ...
        g.text(font, name, x + 14, textY, c.textPrimary(), false);
        boolean unbound = fe.mapping.isUnbound();
        String keyFit = KeybindViewerScreen.fitToWidth(font, key, keyW);
        g.text(font, keyFit, x + w - font.width(keyFit) - 4, textY,
    ...
    private void renderFooter(GuiGraphicsExtractor g) {
        ...
        g.text(font, KeybindViewerScreen.fitToWidth(font, hint, width - 24),
                12, y + (FOOTER_H - font.lineHeight) / 2, c.textSecondary(), false);
    }

    private void renderDragChip(GuiGraphicsExtractor g) {
        var c = UITheme.colors();
        int pad = 6;
        ...
        g.text(font, dragLabel, x + pad, y + (h - font.lineHeight) / 2, c.textPrimary(), false);
    }

    /** Drop target shown while dragging — releasing a function here clears its binding. */
    private void renderUnbindZone(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        var c = UITheme.colors();
        ...
        g.text(font, KeybindViewerScreen.fitToWidth(font, label, r[2] - 10),
    ...
    private void renderNotice(GuiGraphicsExtractor g) {
        ...
        g.text(font, noticeMessage, x + 12, y + 6, c.textPrimary(), false);
    }
    ...
    private void renderCallouts(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        calloutHits.clear();
    ...
    private void renderAnnotationSummary(GuiGraphicsExtractor g, int total, int conflicts, int hidden) {
        var c = UITheme.colors();
        int x = boardX + 2;
        int y = boardTop + 2;
        int w = boardW - 4;
        ...
        g.text(font, KeybindViewerScreen.fitToWidth(font, summary, maxTextW),
    ...
    private void placeBand(GuiGraphicsExtractor g, List<CalloutItem> items, boolean topSide,
    ...
    private void placeColumn(GuiGraphicsExtractor g, List<CalloutItem> items, boolean leftSide,
                             int chipH, int mouseX, int mouseY) {
        int n = items.size();
        if (n == 0) return;
    ...
    private void renderCalloutChip(GuiGraphicsExtractor g, CalloutItem item, int x, int y, int w, int h,
        ...
        g.text(font, fitted, x + 5, y + (h - font.lineHeight) / 2 + 1, c.textPrimary(), false);
        calloutHits.add(new CalloutHit(x, y, w, h, item.mappingName(), item.glfw()));
    }

    private static void fillSeg(GuiGraphicsExtractor g, int x1, int x2, int y, int color) {
        g.fill(Math.min(x1, x2), y, Math.max(x1, x2) + 1, y + 1, color);
    }

    private static void fillVSeg(GuiGraphicsExtractor g, int x, int y1, int y2, int color) {
        g.fill(x, Math.min(y1, y2), x + 1, Math.max(y1, y2) + 1, color);
    }

    private void renderHoverDetails(GuiGraphicsExtractor g, int mouseX, int mouseY) {
    ...
    private int legendItem(GuiGraphicsExtractor g, int x, int y, int swatch, String key) {
        var c = UITheme.colors();
        ...
        g.text(font, label, x + 12, y, c.textSecondary(), false);
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean isDoubleClick) {
        ...
        if (super.mouseClicked(fixedMouseEvent(mx, my, event), isDoubleClick)) return true;
        if (button == 0) {
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        ...
        // Delegate to FixedScaleScreen.mouseDragged (it rescales the raw event coords + drag deltas).
    ...
    public boolean mouseReleased(MouseButtonEvent event) {
        ...
        return releaseLogicalMouse(mx, my, event);
    }
    ...
    private void bindToKey(KeyMapping mapping, int glfwKey) {
        ...
        KeybindPriorityEnforcer.rebind(mapping, key);

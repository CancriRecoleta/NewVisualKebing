...
public class KeybindViewerScreen extends FixedScaleScreen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        super.extractRenderState(g, fixedMouseX, fixedMouseY, partialTick);
    ...
    private void renderStatusBar(GuiGraphicsExtractor g, int mouseX, int mouseY) {
        var c = UITheme.colors();
    ...
    private void renderStatusChip(GuiGraphicsExtractor g, int x, int y, int w, int h,
    ...
    private void renderLayoutEditOverlay(GuiGraphicsExtractor g, int mouseX, int mouseY) {
...
static int paintPanelBase(GuiGraphicsExtractor g, net.minecraft.client.gui.Font font, int x, int y, int w, int h, String title) {
     ...
     */
    private void renderInlineLegend(GuiGraphicsExtractor g, int x, int y, int w, int h) {
        var c = UITheme.colors();
        ...
        if (legendTitle != null && !legendTitle.isEmpty()) {
            ...
            if (cx + tw + 10 < right) {
                g.text(font, legendTitle, cx, cy, c.textMuted(), false);
                cx += tw + 10;
            }
        }
        for (int i = 0; i < legendLabels.length; i++) {
            ...
            g.text(font, legendLabels[i], cx + sw + gap, cy, c.textSecondary(), false);
            cx += itemW + itemGap;
        }
    }
    ...
    private void renderKeyboardBottomBand(GuiGraphicsExtractor g, int x, int y, int w, int h, Integer virtualKey) {
        ...
        int textY = y + (h - font.lineHeight) / 2;
        if (virtualKey == null) {
            g.text(font, fitToWidth(font, bandHoverHint, w - 18), x + 9, textY, c.textMuted(), false);
            return;
        }
        ...
        int right = x + w - 8;
        if (bindings.isEmpty()) {
            g.text(font, fitToWidth(font, bandUnboundLabel, right - curX), curX, textY, c.textMuted(), false);
            return;
        }
    ...
    private void renderCollapsedPanel(GuiGraphicsExtractor g, int x, int y, int w, String title,
                                      int mouseX, int mouseY, boolean isMouse) {
        var c = UITheme.colors();
        UITheme.drawGlassPanel(g, x, y, w, COLLAPSED_PANEL_H, PANEL_RADIUS);
        g.text(font, fitToWidth(font, title, w - PANEL_PAD * 2 - PANEL_TOGGLE_SIZE - 4),
     ...
     */
    private void drawPanelToggle(GuiGraphicsExtractor g, int x, int y, int w, int headerH,
    ...
    private static void drawChevron(GuiGraphicsExtractor g, int cx, int cy, boolean down, int color) {
        int oy = down ? -1 : 1;
        for (int i = 0; i <= 3; i++) {
            int dy = down ? i : -i;
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean isDoubleClick) {
        ...
        if (layoutEditMode) {
            ...
            return super.mouseClicked(fixedMouseEvent(mouseX, mouseY, event), isDoubleClick);
        }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
        ...
        if (Minecraft.getInstance().hasAltDown() && overKeyboard(mouseX, mouseY)) {
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        ...
        if (draggingPanel != null) {
            double mouseX = fixedMouseX(event.x());
            double mouseY = fixedMouseY(event.y());
            int rawOffsetX = dragBaseOffsetX + (int) Math.round(mouseX - dragGrabX);
            int rawOffsetY = dragBaseOffsetY + (int) Math.round(mouseY - dragGrabY);

...
final class KeybindTooltipRenderer {
    ...
    void render(GuiGraphicsExtractor g, Font font, int screenW, int screenH, int virtualKey, int mouseX, int mouseY) {
        ...
        if (layout.isWheel()) {
            ...
            for (BindingRowLayout row : layout.rows()) {
                ...
                for (int i = 0; i < actionLines.size(); i++) {
                    g.text(font, actionLines.get(i), curX + 8, ly,
                            info.self() ? c.accent() : c.textPrimary(), true);
                    if (i == 0) {
                        int rightX = curX + innerW - 6 - row.modW();
                        g.text(font, row.modText(), rightX, ly, c.textSecondary(), true);
                        if (!row.ctxTag().isEmpty()) {
                            ...
                            g.text(font, row.ctxTag(), tagX, ly, c.accentAlt(), true);
                        }
                    }
                    ly += lineH;
                }
                g.text(font, row.metaFit(), curX + 8, ly, c.textMuted(), true);
                ly += lineH;
                // Translation key wraps on its own (mod ids can be long); the key pair sits below it.
                for (String idLine : row.idLines()) {
                    g.text(font, idLine, curX + 8, ly, UITheme.withAlpha(c.textMuted(), 0xBE), true);
                    ly += lineH;
                }
                g.text(font, row.keyInfoFit(), curX + 8, ly,
        ...
        if (status == KeyBindingScanner.KeyStatus.CONFLICT) {
            ...
            g.text(font, layout.conflictFit(), curX + 7, curY + 1, c.dangerColor(), true);
            curY += warnH + 3;
        }
        ...
        if (comboLines.length > 0) {
            ...
            for (String line : comboLines) {
                ...
                g.text(font, line, curX + 7, curY + 2, yellow, true);
                curY += lineH + 2;
            }
            curY += 2;
        }

...
public class KeybindComboManageScreen extends FixedScaleScreen {
    ...
    private void renderHeader(GuiGraphicsExtractor graphics) {
        ...
        graphics.text(font, KeybindViewerScreen.fitToWidth(font, count, Math.max(40, titleRight - titleX)),
    ...
    private void renderRow(GuiGraphicsExtractor graphics, KeybindComboStore.ComboBinding combo,
        ...
        graphics.text(font, name, textColX, textTop, colors.textPrimary(), false);
        int comboY = textTop + font.lineHeight + 3;
        graphics.text(font, comboLabel, textColX, comboY, HIGHLIGHT_COLOR, false);
    ...
    private void renderCaptureOverlay(GuiGraphicsExtractor graphics) {
        var colors = UITheme.colors();
        ...
        if (capture.stage == CaptureStage.SELECT_MAPPING) {
            ...
            graphics.text(font, l2, bx + (bw - font.width(l2)) / 2, by + 36, colors.textPrimary(), true);
            graphics.text(font, l3, bx + (bw - font.width(l3)) / 2, by + 56, HIGHLIGHT_COLOR, false);
            String tip = Component.translatable("screen.newvisualkeybing.viewer.combo.cancel_hint").getString();
            graphics.text(font, tip, bx + (bw - font.width(tip)) / 2, by + bh - 16, colors.textMuted(), false);
        }
    }

    private void renderMappingPicker(GuiGraphicsExtractor graphics, int bx, int by, int bw, int bh) {
        var colors = UITheme.colors();
        String search = capture.search.isEmpty()
                ? Component.translatable("screen.newvisualkeybing.viewer.combo.search").getString()
                : "> " + capture.search;
        graphics.text(font, search, bx + 12, by + 22,
        ...
        for (int i = capture.scrollOffset; i < mappings.size() && i < capture.scrollOffset + visible; i++) {
            ...
            graphics.text(font, row, listX + 8, drawY, colors.textPrimary(), false);
            drawY += rowH;
        }
        graphics.disableScissor();
    ...
    public boolean keyPressed(KeyEvent event) {
        ...
        if (capture != null) {
            ...
            onKeyCaptured(InputConstants.getKey(event));
            return true;
        }
        if (keyCode == GLFW.GLFW_KEY_ESCAPE) {
            if (mappingSearchBox != null && mappingSearchBox.isFocused()) {
    ...
    public boolean charTyped(CharacterEvent event) {
        ...
        if (capture != null) {
            ...
            if (capture.stage == CaptureStage.SELECT_MAPPING && codePoint >= 32) {
                capture.search += event.codepointAsString();
                capture.scrollOffset = 0;
            }
            return true;
        }
    ...
    private void onKeyCaptured(InputConstants.Key key) {
        ...
        KeybindPriorityEnforcer.rebind(capture.mapping, key);

...
public final class KeybindPriorityEnforcer {
    ...
    /**
     * Rebind {@code mapping} to {@code key} and re-publish. Replaces vanilla {@code Options.setKey},
     * which was removed in 1.21.2: set the key on the mapping, persist the options file, then
     * re-publish via {@link #resetAndEnforce()} (whose {@code resetMapping} mixin TAIL invalidates the
     * caches and reconciles combos). Faithfully mirrors the old {@code Options.setKey} behaviour the
     * screens relied on.
     */
     ...
     * Ctrl&gt;Shift&gt;Alt precedence (via {@link Minecraft#hasControlDown()} etc., which work on both
    ...
    /** Forge's active-modifier precedence. 1.21.10: the static {@code Screen.has*Down} helpers were
     *  removed; the live modifier state now lives on {@link Minecraft}. */
    private static InputModifier currentModifier() {
        Minecraft mc = Minecraft.getInstance();
        if (mc.hasControlDown()) return InputModifier.CONTROL;
        if (mc.hasShiftDown()) return InputModifier.SHIFT;
        if (mc.hasAltDown()) return InputModifier.ALT;

...
public class KeybindProfileImportScreen extends FixedScaleScreen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        try {
            ...
            super.extractRenderState(graphics, fmx, fmy, partialTick);
    ...
    private void renderHeader(GuiGraphicsExtractor graphics) {
        ...
        graphics.text(font, title, 16, titleY, colors.textPrimary(), false);
        graphics.text(font, count, 16, titleY + font.lineHeight + 3, colors.textSecondary(), false);
    }
    ...
    private void renderList(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        ...
        if (entries.isEmpty()) {
            ...
            graphics.text(font, empty, x + (w - font.width(empty)) / 2,
    ...
    private void renderRow(GuiGraphicsExtractor graphics, KeybindProfileStore.ExportEntry entry,
        ...
        graphics.text(font, KeybindViewerScreen.fitToWidth(font, name, w - 200),
        ...
        graphics.text(font, meta, textX, y + 6 + font.lineHeight + 3, colors.textSecondary(), false);
        ...
        graphics.text(font, when, x + w - whenW - 12, y + 6, colors.textMuted(), false);

        String fileName = entry.path.getFileName().toString();
        int fileNameW = font.width(fileName);
        graphics.text(font, KeybindViewerScreen.fitToWidth(font, fileName, 240),
    ...
    private void renderNotice(GuiGraphicsExtractor graphics) {
        ...
        graphics.text(font, notice, x + pad, y + 7, colors.textPrimary(), false);
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean isDoubleClick) {
        applyFixedScaleMetrics();
        double fmx = fixedMouseX(event.x());
        double fmy = fixedMouseY(event.y());
        int top = listTop();
        int h = listHeight();
        int x = listX();
        int w = listW();
        ...
        return super.mouseClicked(fixedMouseEvent(fmx, fmy, event), isDoubleClick);
    }
